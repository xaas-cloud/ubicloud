...
add element inet drop_unused_ip_packets allowed_ipv4_addresses { #{ip_net} }
  ...
  def setup_taps_6(gua, nics)
    ...
    r "ip -n #{q_vm} addr replace fd00:0b1c:100d::/48 dev #{nics.first.tap}"
  end
...
bootcmd:
  ...
  - [nft, add, rule, ip6, filter, output, ip6, daddr, fd00:0b1c:100d::/48, meta, skuid, "!=", 0, tcp, flags, syn, reject, with, tcp, reset]
