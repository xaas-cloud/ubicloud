...
WHERE (SELECT max(available_storage_gib) FROM storage_device WHERE storage_device.enabled AND storage_device.vm_host_id = vm_host.id) >= #{total_storage_gib} AND
  ...
  label def prep
    ...
    when "NotStarted", "Failed"
      ...
      host.sshable.cmd("common/bin/daemonizer 'sudo host/bin/setup-vm prep #{params_path.shellescape}' prep_#{q_vm}", stdin: secrets_json)
    end
  ...
  label def destroy
    ...
    unless host.nil?
      ...
      host.sshable.cmd("sudo host/bin/setup-vm delete #{params_path.shellescape}")
    end
  ...
  label def start_after_host_reboot
    ...
    host.sshable.cmd("sudo host/bin/setup-vm recreate-unpersisted #{params_path.shellescape}", stdin: secrets_json)
    host.sshable.cmd("sudo systemctl start #{q_vm}")
