...
RSpec.describe Prog::Vm::Nexus do
  ...
  describe "#prep" do
    ...
    it "generates and passes a params json if prep command is not started yet" do
      ...
      expect(sshable).to receive(:cmd).with(/sudo host\/bin\/setup-vm prep/, {stdin: /{"storage":{"vm.*_0":{"key":"key","init_vector":"iv","algorithm":"aes-256-gcm","auth_data":"somedata"}}}/})
  ...
  describe "#destroy" do
    ...
    context "when has vm_host" do
      ...
      it "absorbs an already deleted errors as a success" do
        ...
        expect(sshable).to receive(:cmd).with(/sudo.*bin\/setup-vm delete \/vm\/#{nx.vm_name}\/prep.json/)
  ...
  describe "#start_after_host_reboot" do
    ...
    it "can start a vm after reboot" do
      expect(sshable).to receive(:cmd).with(
        /sudo host\/bin\/setup-vm recreate-unpersisted \/vm\/vm[0-9a-z]+\/prep.json/,
